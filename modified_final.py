...
# Define helper function to plot graph with axis labels
def plot_graph(figsize, values, full_data, extra_data=0, extra_dataset=None, x_label='', y_label=''):
    fig = plt.figure(figsize=figsize)
    plt.plot(values, 'Orange', label="Predicted")
    plt.plot(full_data.Close, 'b', label="Actual")
    if extra_data:
...
for days in [250, 200, 100]:
    ...
    st.pyplot(plot_graph((15, 6), stock_data[f"MA_for_{days}_days"], stock_data, 0, x_label='Days', y_label='Stock Price'))
...
predicted_df = pd.DataFrame({
    "Prediction Date": prediction_range,
    "Predicted Stock Price": predictions
})
st.subheader("Predicted Stock Prices")
st.write(predicted_df)

# Plot prediction results with X and Y axis labels
st.subheader(f"Predictions from {start_prediction} to {end_prediction}")
st.line_chart(predictions)

# Plot graph for the predicted range with proper axis labels
st.pyplot(plot_graph((15, 6), predictions, stock_data, 0, x_label="Date", y_label="Predicted Stock Price"))
